...
这些不再是拍脑袋的经验值，已经落成可计算、可留痕、可复盘的**交易纪律引擎**
（`src-tauri/src/discipline/`，界面在侧栏「交易纪律」）：

| 项目 | 标准档默认值 |
|------|------------|
| 单笔最大亏损 | 总资产 **2%**，由此反推可买股数 |
| 止损线 | 固定 8% / 3.5×ATR / 支撑位下方 三者取最严，之后只上移不下移 |
| 最低盈亏比 | 2:1（买入前必须填止损价与目标价，否则拒绝） |
| 移动止盈 | 浮盈 10% 后，从持仓最高价回撤 8% 清仓 |
| 分批止盈 | +15% / +30% 各减半 |
| 时间止损 | 持有 20 个交易日仍未达 +3% 即换股 |
| 单票 / 总仓 / 只数上限 | 25% / 80% / 5 只 |
| 冷静期 / 熔断 | 止损后 10 个交易日禁买；连亏 3 笔停手 5 个交易日 |

数学含义：单笔风险固定 2% + 盈亏比门槛 2:1 → 胜率 40% 即正期望，连输 10 笔才亏 20%。
阈值均可在界面调整；调整前请先读 `discipline/rules.rs` 的模块注释，里面写了每个数字
的来历——它们是行业经验值，**不是从本项目数据优化出来的**，本系统也刻意不提供参数寻优
（几十笔样本上寻优等于过拟合）。

详见 [README](./README.md) 的「交易纪律」章节。
