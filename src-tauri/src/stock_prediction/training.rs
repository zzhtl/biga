...
use crate::stock_prediction::technical_indicators::{
    calculate_feature_value, get_feature_required_days,
};
use crate::stock_prediction::types::{ModelConfig, ModelInfo, TrainingRequest, TrainingResult};
use crate::stock_prediction::utils::{
    calculate_direction_focused_accuracy, smooth_price_data, smooth_volume_data,
};
...
fn create_model(
    config: &ModelConfig,
    device: &Device,
) -> Result<(VarMap, Box<dyn Module + Send + Sync>), candle_core::Error> {
...
async fn prepare_stock_data(
    ...
    let user_end_date =
        chrono::NaiveDate::parse_from_str(&request.end_date, "%Y-%m-%d").unwrap_or(end_date);
    ...
    // 使用sqlx查询数据库获取历史数据
    let historical_data =
        match get_historical_data_from_db(symbol, &start_date_str, &end_date_str).await {
            Ok(data) => data,
            Err(e) => {
    ...
    if valid_data.len() < recommended_days {
        println!(
            "⚠️  警告: 当前数据量{}天少于推荐的{}天，可能影响模型准确率",
            valid_data.len(),
            recommended_days
        );
    } else if valid_data.len() >= optimal_days {
        println!(
            "✅ 数据量充足: {}天 >= {}天，有利于提高模型准确率",
            valid_data.len(),
            optimal_days
        );
    }
    ...
    let x_train_vec: Vec<f64> = normalized_features[0..train_size]
        .iter()
    ...
    let x_test_vec: Vec<f64> = normalized_features[train_size..]
        .iter()
        .flat_map(|v| v.iter().copied())
        .collect();
    ...
    println!(
        "数据预处理完成: 训练集{train_size}样本, 测试集{test_size}样本, 特征维度{features_len}"
    );
...
fn normalize_features(
    features: &[Vec<f64>],
) -> Result<(Vec<Vec<f64>>, Vec<(f64, f64)>), candle_core::Error> {
...
// 训练模型函数
pub async fn train_candle_model(
    request: TrainingRequest,
) -> std::result::Result<TrainingResult, String> {
    ...
    // 创建模型
    let (varmap, model) =
        create_model(&config, &device).map_err(|e| format!("模型创建失败: {e}"))?;

    // 创建优化器（使用AdamW，更适合深度网络）
    let initial_lr = request.learning_rate;
    let mut optimizer =
        AdamW::new_lr(varmap.all_vars(), initial_lr).map_err(|e| format!("优化器创建失败: {e}"))?;
    ...
    for epoch in 0..request.epochs {
        ...
        for batch_idx in 0..num_batches {
            ...
            let x_batch = x_train
                .narrow(0, batch_start, batch_size)
        ...
        if epoch == 0
            || (epoch + 1) % 5 == 0
            || epoch == request.epochs - 1
            || patience_counter >= patience
        {
            println!(
                "Epoch {:3}/{} | 训练Loss: {:.6} | 验证Loss: {:.6} | 最佳Loss: {:.6} | 耐心: {}/{}",
                epoch + 1,
                request.epochs,
                avg_train_loss,
                val_loss,
    ...
    let (direction_accuracy, combined_accuracy) =
        calculate_direction_focused_accuracy(&predictions_vec, &actuals_vec);

    // 计算MSE和RMSE用于日志显示
    let diff = y_pred
        .sub(&y_test)
        .map_err(|e| format!("计算MSE失败: {e}"))?;
