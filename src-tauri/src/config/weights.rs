...
// =============================================================================
//
// 这两个系数决定「量比」和「换手率」对量价因子评分的影响幅度，
// 是 v0.3 新增的两个关键调参旋钮，直接影响放量/缩量与流动性对预测的贡献。
//
// 计算位置：prediction/strategy/multi_factor/factors.rs::calculate_volume_price_score_enhanced

/// 量比影响系数：放量配合方向时的最大加/减分幅度（0-1 评分尺度）
/// - 放量上涨 +IMPACT；缩量上涨 -0.6×IMPACT
/// - 放量下跌 -IMPACT；缩量下跌 +0.5×IMPACT
pub const VOLUME_RATIO_IMPACT: f64 = 0.10;

/// 换手率影响系数：流动性健康度对量价评分的最大调整幅度（0-1 评分尺度）
/// - 适度换手(2%-15%) +0.75×IMPACT；过热换手(>15%) -IMPACT
pub const TURNOVER_RATE_IMPACT: f64 = 0.04;
